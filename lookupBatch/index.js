...
// ---- ACS Operator Information Search (single number per request) ----
async function numberLookup(acsEndpoint, acsAccessKeyBase64, number) {
  ...
  // Normalize to an array of one result
  return Array.isArray(parsed.values) ? parsed.values : parsed.value ? [parsed.value] : [parsed];
}
...
module.exports = async function (context, req) {
  try {
    ...
    // Call ACS one number at a time (per server requirement)
    const allResults = [];
